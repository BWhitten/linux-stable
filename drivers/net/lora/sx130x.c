...
static int sx130x_tx(struct sx130x_priv *priv, struct sk_buff *skb)
{
	...
	hdr->tx_freq[0] &= 0x3F;
	if (cfg_bandwidth == 125) {
		hdr->tx_freq[0] |= 0x40;
	} else if (cfg_bandwidth == 500) {
		hdr->tx_freq[0] |= 0x80;
	}
...
static int sx130x_loradev_open(struct net_device *netdev)
{
	...
	x = 4096000000UL / (868500000UL >> 3); /* dividend: (16*2048*1000000) >> 3, rescaled to avoid 32b overflow */
	x = ( x > 63 ) ? 63 : x; /* saturation */
...
int sx130x_lora_get_sf(struct lora_phy *phy, u8 *val)
{
	struct sx130x_priv *priv = netdev_priv(phy->netdev);
...
int sx130x_lora_set_sf(struct lora_phy *phy, u8 val)
{
	struct sx130x_priv *priv = netdev_priv(phy->netdev);
...
int sx130x_lora_get_tx_power(struct lora_phy *phy, s32 *val)
{
	...
	mutex_lock(&priv->cfg_lock);
	if (val)
		*val = priv->cfg_tx_power;
	mutex_unlock(&priv->cfg_lock);

	return 0;
}

int sx130x_lora_set_tx_power(struct lora_phy *phy, s32 val)
{
	struct sx130x_priv *priv = netdev_priv(phy->netdev);
