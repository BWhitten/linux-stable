...
static int sx1301_load_tx_gain_lut(struct sx1301_priv *priv)
{
	...
	for (i = 0; i < priv->tx_gain_lut_size; i++) {
		val = lut->mix_gain + (lut->dac_gain << 4) + (lut->pa_gain << 6);
		...
		if (val != (0x30 + i)) {
			dev_err(priv->dev, "AGC firmware LUT init error: 0x%02X", val);
			return -ENXIO;
		}
		lut++;
	}
	...
	if (priv->tx_gain_lut_size < SX1301_TX_GAIN_LUT_MAX) {
		...
		if (val != 0x30) {
			dev_err(priv->dev, "AGC firmware LUT abort error: 0x%02X", val);
			return -ENXIO;
		}
	}
...
static int sx130x_loradev_open(struct net_device *netdev)
{
	...
	if (val != 0x30) {
		dev_err(priv->dev, "AGC firmware chan select error: 0x%02X", val);
		return -ENXIO;
	}
	...
	priv->wq = alloc_workqueue("sx1301_wq", WQ_FREEZABLE | WQ_MEM_RECLAIM, 0);
